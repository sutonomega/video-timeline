...
5. batch CLIは `--input-dir` を必須とする。`video_timeline.toml` がある場合、ファイル名だけの `--input-dir` と `--output-dir` は `storage.root` 配下へ解決する。`--output-dir` を省略した場合は `<storage.root>/<storage.timelines_dir>` を使う。設定ファイルがない場合は従来通り `--output-dir` を必須にする。
...
現時点では`timeline`と`events`は近い構造で、`summary`、`start_seconds`、`end_seconds`はほぼ同じ情報を持つ。これは意図的な暫定形であり、今の実装では`events`を削除せず、後続で`coding`、`chat`、`browser`などの詳細分類やLLMによる重要イベント抽出を追加できる余地として残す。ただし、`events`をtimelineの完全な複製として育てない。`events`に追加する情報は、`kind`、`importance_score`、分類理由、検索UI向けの強調情報など、timeline区間そのものとは異なる判断結果に限定する。

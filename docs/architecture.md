...
HTML出力CLIはWebUI前段の確認用とし、生成済み`timeline.json`をブラウザで素早く眺めるための最小表示にする。時刻表記は検索CLIと同じく小数秒を切り捨て、`01:20-04:10`のように表示する。`timeline`がないJSONはエラーにし、`events`がないJSONは空の一覧として扱う。
